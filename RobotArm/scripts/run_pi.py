...
#     print 'addr = ', addr 
    ...
    # run Gse GUI
    GUI_args = [python_bin,"%s/Gse/bin/gse.py"%build_root,"--port","%d"%used_port,"--dictionary","%s/Gse/generated/RobotArm"%build_root,"--connect","--addr",addr,"-L","%s/RobotArm/logs"%build_root]
    ...
    pi_bin = "%s/RobotArm/%s/RobotArm"%(build_root,os.environ["OUTPUT_DIR"])
    
    # run as root
    if not nobin:
        REF_args = [python_bin, "%s/Gse/bin/pexpect_runner.py"%build_root,"RobotArm.log","RobotArm Application","sudo",pi_bin,"-p","%d"%used_port,"-a",addr]
